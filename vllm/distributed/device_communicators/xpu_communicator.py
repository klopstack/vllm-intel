...
class XpuCommunicator(DeviceCommunicatorBase):
    ...
    def _init_host_staging(self):
        ...
        if _HOST_STAGED:
            logger.info(
                "XpuCommunicator: host-staged collectives enabled (threshold=%d bytes)",
                _HOST_STAGED_MIN_BYTES,
            )

    def _needs_staging(self, t: torch.Tensor) -> bool:
        return _HOST_STAGED and t.numel() * t.element_size() >= _HOST_STAGED_MIN_BYTES
    ...
    def _staged_all_gather(self, output: torch.Tensor, input_: torch.Tensor) -> None:
        inbuf = self._pinned(input_.shape, input_.dtype)
        inbuf.copy_(input_, non_blocking=False)
        outbuf = self._pinned((self.world_size,) + tuple(inbuf.shape), input_.dtype)
    ...
    def _staged_reduce_scatter(self, output: torch.Tensor, input_: torch.Tensor) -> None:
        inbuf = self._pinned(input_.shape, input_.dtype)
        inbuf.copy_(input_, non_blocking=False)
